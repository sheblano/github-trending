  ...
  withMethods((store) => {
    ...
    return {
      ...
      async loadStarred(): Promise<void> {
        ...
        try {
          const res = await firstValueFrom(api.getStarred(1, STARRED_PER_PAGE));
          patchState(store, {
      ...
      async loadReleases(): Promise<void> {
        ...
        try {
          const res = await firstValueFrom(api.getReleases(1, RELEASES_PER_PAGE));
          patchState(store, {
      ...
      async loadMoreReleases(): Promise<void> {
        ...
        try {
          const res = await firstValueFrom(api.getReleases(nextPage, RELEASES_PER_PAGE));
          patchState(store, {
      ...
      async updateNotes(repoId: number, notes: string | null): Promise<void> {
        try {
          await firstValueFrom(api.updateNotes(repoId, notes));
      ...
      async unstar(owner: string, name: string): Promise<void> {
        try {
          await firstValueFrom(api.unstarRepo(owner, name));
          const repos = store
            .starredRepos()
